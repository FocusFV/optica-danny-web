  ...
  useEffect(() => {
    ...
    async function cargarDetalle() {
      try {
        ...
        if (!res.ok) throw new Error("No se encontró el armazón");
  ...
  if (!armazon) {
    return (
      <div className="min-h-screen bg-[#001529] text-white flex flex-col items-center justify-center p-6">
        <h2 className="text-2xl font-bold text-amber-500 mb-4">El armazón no existe</h2>
        ...
        <div className="flex flex-col justify-between">
          <div>
            <span className="text-[10px] text-[#c5a059] font-black uppercase tracking-[0.3em]">
              {armazon.stock > 0 ? `Disponibles en Óptica: ${armazon.stock} un.` : "Agotado"}
            </span>
            ...
            <p className="text-neutral-300 text-sm leading-relaxed mb-6">
              {armazon.descripcion || "Diseño ergonómico exclusivo de Óptica Danny."}
            </p>
          </div>
          ...
          <div>
            ...
            <div className="flex gap-4">
              <button onClick={() => router.back()} className="w-1/3 border border-white/10 text-white/70 text-xs font-bold uppercase tracking-widest p-3 rounded-xl hover:bg-white/5 transition-all">
                Volver
              </button>
              <Link href="/#contacto" className="w-2/3 bg-[#c5a059] hover:bg-[#b08e4f] text-black font-black text-xs uppercase tracking-widest p-3 rounded-xl transition-all text-center flex items-center justify-center">
                Consultar WhatsApp
              </Link>
            </div>
          </div>
