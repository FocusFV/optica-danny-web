            ...
            <div key={i} className="group relative">
              ...
              >
                ...
                <div className="space-y-4">
                  ...
                  <p className="text-gray-400 text-xs uppercase tracking-widest font-light leading-relaxed opacity-70">
                    {member.bio}
                  </p>
                </div>
              </div>
              ...
              <div className="flex gap-6 pt-6 items-center">
                 ...
                 {member.ig && (
                   <a href={member.ig} target="_blank" rel="noopener noreferrer" className="text-white/40 hover:text-[#c5a059] transition-all"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"></rect><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"></path><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"></line></svg></a>
                 )}
                 {member.in && (
                   <a href={member.in} target="_blank" rel="noopener noreferrer" className="text-white/40 hover:text-[#c5a059] transition-all"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"></path><rect x="2" y="9" width="4" height="12"></rect><circle cx="4" cy="4" r="2"></circle></svg></a>
                 )}
              </div>
            </div>
          ))}
        ...
        <AnimatePresence>
          <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 md:p-6 pointer-events-auto">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-3xl bg-zinc-950 border border-white/10 rounded-[2.5rem] overflow-hidden shadow-2xl z-[100000]"
            >
              <button onClick={() => setSelectedMember(null)} className="absolute top-8 right-8 text-white/50 hover:text-white z-50 transition-colors"><X size={32} /></button>
              
              <div className="flex flex-col md:flex-row max-h-[90vh] overflow-y-auto md:overflow-hidden">
                <div className="w-full md:w-[40%] aspect-[4/5] md:aspect-auto">
                  <img src={team[selectedMember].image} alt="" className="w-full h-full object-cover grayscale brightness-75" />
                </div>
                
                <div className="p-8 md:p-12 flex-1 flex flex-col justify-center">
                  <span className="text-[#c5a059] text-[10px] font-black uppercase tracking-[0.5em]">{team[selectedMember].role}</span>
                  ...
                  <p className="text-gray-300 text-sm md:text-base leading-relaxed font-light mb-10">
                    {team[selectedMember].fullBio}
                  </p>

                  <div className="grid grid-cols-1 gap-4">
                    {team[selectedMember].highlights.map((h, idx) => (
                      <div key={idx} className="flex items-center gap-3 text-[10px] text-white uppercase tracking-[0.3em] font-bold opacity-80">
